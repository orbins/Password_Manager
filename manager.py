            ...
            if password == password2:
                ...
                if updated_service:
                    self.db_manager.update_service(
                        userid, updated_service, service_name, login, token, info
                    )
                else:
                    self.db_manager.add_service(
                        userid, service_name, login, token, info
                    )
    ...
    def generate_password(self):
        ...
        if length.isdigit() and (6 <= int(length) <= 20):
            ...
            while True:
                ...
                pattern = (
                    r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_])[a-zA-Z\d\W_]{6,20}$"
                )
